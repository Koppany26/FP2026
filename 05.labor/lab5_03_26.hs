...
main = do
    -- let lista="egy PrOBA szoveg. ezz egy masik azz proBa! Tobbfele irasJEL ::Hasznalat";
    -- let l1= tokenize lista;
    -- let l2=length_Lista l1;
    -- let m1=minimum l2;
...
--   337
--   ```
populacio ls=sum[t2 | (t1,t2,t3)<- ls]

populacio2 ls regio =sum [t2 | (t1,t2,t3)<- ls1]
  where 
    ls1= filter (\ (t1,t2,t3)-> t3==regio)ls

main2 = do
  let ls = [("golya",120,"MS"),("fecske",85,"CJ"),("cinege",132,"MS")]
  let res = populacio ls 
  let madarLS = concatMap (\s -> s <> " ") [t1 | (t1,t2,t3) <- ls]
  
  putStrLn $ madarLS <> " populacio szama: " <> show res
  let result= populacio2 ls "MS"
  putStrLn $ madarLS <> " " <>"MS"<> "-ben a populacio szam: " ++ show result
